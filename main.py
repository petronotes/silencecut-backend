"""SilenceCut Backend — FastAPI + FFmpeg v4"""
...
def normalize_to_mkv(input_path: Path, work_dir: Path) -> Path:
    """
    Нормализация через pipe: decode → rawvideo+pcm → encode в MP4 baseline.
    Pipe передаёт видео и аудио раздельно через временные файлы FIFO/pipe.
    Используем yuv420p rawvideo с правильным размером кадра из ffprobe.
    """
    norm_path = work_dir / "norm.mp4"

    # Получаем реальные размеры кадра и rotation
    ...
    # Определяем rotation из side_data или тегов
    rotation = 0
    for sd in vs.get("side_data_list", []):
        if sd.get("side_data_type") == "Display Matrix":
            ...
            break
    if rotation == 0:
        rot_tag = vs.get("tags", {}).get("rotate", "0")
        try: rotation = int(rot_tag)
        except: rotation = 0

    # Реальные размеры кадра из потока
    w_raw = int(vs.get("width",  1080))
    h_raw = int(vs.get("height", 1920))

    # После применения rotation меняются ширина/высота
    if abs(rotation) in (90, 270):
        w, h = h_raw, w_raw   # поворот 90/270 → меняем местами
    else:
        w, h = w_raw, h_raw
    ...
    # vf: сначала поворот (autorotate), затем scale до чётных значений
    # autorotate читает displaymatrix и применяет transpose автоматически
    vf_filter = f"scale={w}:{h}"

    raw_video = work_dir / "raw_video.yuv"
    raw_audio = work_dir / "raw_audio.pcm"

    # Шаг A: декодируем видео в rawvideo файл с применением rotation
    cmd_vid = [
        "ffmpeg", "-y",
        "-fflags", "+genpts+igndts+discardcorrupt",
        "-err_detect", "ignore_err",
        "-ec", "deblock",
        "-noautorotate",               # отключаем авторотацию — управляем сами
        "-i", str(input_path),
        "-an",
        "-f", "rawvideo",
        "-pix_fmt", "yuv420p",
        "-vf", f"transpose=1,scale={w}:{h}" if abs(rotation) in (90, 270) else vf_filter,
        "-r", "30",
        "-vsync", "cfr",
        str(raw_video),
    ]
    run_cmd(cmd_vid, timeout=900)

    # Шаг B: декодируем аудио в raw PCM файл
    cmd_aud = [
        "ffmpeg", "-y",
        "-fflags", "+genpts+igndts+discardcorrupt",
        "-i", str(input_path),
    ...
    run_cmd(cmd_aud, timeout=300)

    # Шаг C: кодируем rawvideo + raw PCM → финальный norm.mp4
    cmd_enc = [
        "ffmpeg", "-y",
        # Видео из rawvideo файла
        "-f", "rawvideo",
        "-pix_fmt", "yuv420p",
        "-video_size", f"{w}x{h}",
        "-framerate", "30",
        "-i", str(raw_video),
        # Аудио из raw PCM файла
        "-f", "s16le",
        "-ar", "48000",
        "-ac", "2",
        "-i", str(raw_audio),
    ...
    if not norm_path.exists() or norm_path.stat().st_size < 10000:
        raise RuntimeError("Не удалось нормализовать видео.")
...
def process_with_ffmpeg(task_id: str, input_path: Path, output_path: Path, params: dict):
    ...
    # ШАГ 1 — нормализация в MKV baseline с частыми keyframe
    t.update({"log": "Нормализация видео (1/4)…", "progress": 5})
    norm_path = normalize_to_mkv(input_path, work_dir)

    # ШАГ 2 — анализ тишины на нормализованном файле
    t.update({"log": "Анализ аудио (2/4)…", "progress": 35})
    ...
    t.update({"log": f"Сегментов: {len(segs)}, нарезаю (3/4)…", "progress": 45})

    # ШАГ 3 — нарезка через -ss/-t -c copy (norm.mkv baseline → seek точный)
    seg_paths = []
    n = len(segs)
    for i, (start, end) in enumerate(segs):
        ...
        cmd = [
            ...
            "-i", str(norm_path),
            "-t", f"{end - start:.4f}",
            "-c", "copy",             # просто копируем — baseline гарантирует точный seek
            str(seg_path),
        ]
    ...
    t.update({"log": "Склейка → MP4 (4/4)…", "progress": 82})

    # ШАГ 4 — склейка через concat demuxer + финальное кодирование в MP4
    ...
    cmd = [
        ...
        "-safe", "0",
        "-i", str(list_path),
        "-c", "copy",                   # сегменты уже H264 MP4 (AVCC) — просто копируем
        "-movflags", "+faststart",
        str(output_path),
    ]
