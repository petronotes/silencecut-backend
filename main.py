...
def normalize_to_mkv(input_path: Path, work_dir: Path) -> Path:
    """
    Нормализация с обходом битых NAL-единиц Samsung.
    -err_detect ignore_err + -ec deblock — пропускаем битые кадры.
    -skip_frame noref — не декодируем non-reference кадры (ускорение).
    Два прохода: сначала пробуем быстрый, при ошибке — с ignore_err.
    """
    ...
    # Попытка 1: с ignore_err и deblock-восстановлением
    cmd = [
        "ffmpeg", "-y",
    ...
    # Попытка 2: через pipe — декодируем через ffmpeg → pipe → ffmpeg encode
    # Полностью обходит проблему контейнера: читаем сырые кадры через stdout
    ...
    encode_cmd = [
        ...
        "-x264-params", "keyint=30:min-keyint=30:scenecut=0",
        "-pix_fmt", "yuv420p",
        "-an",                          # аудио добавим отдельно
        str(work_dir / "norm_vid.mp4"),
    ]
    ...
    # НЕ закрываем p1.stdout до завершения p2 — иначе EOF раньше времени
    _, err2 = p2.communicate(timeout=7200)
    # Ждём завершения декодера после того как энкодер закончил
    try:
        p1.wait(timeout=120)
    except subprocess.TimeoutExpired:
        p1.kill()
    # Если p1 упал — получаем его stderr
    if p1.returncode not in (0, None, -9):
        p1_err = p1.stderr.read().decode(errors="replace") if p1.stderr else ""
        # не фатально — decode мог завершиться с ненулевым кодом из-за битых кадров
        pass
    if p2.returncode != 0:
        err_text = err2.decode(errors="replace")
        # Ищем реальную причину — последние строки после последнего "frame="
        lines = [l for l in err_text.split("\n") if l.strip() and "frame=" not in l]
        raise RuntimeError("Pipe encode failed: " + "\n".join(lines[-20:]))

    # Извлекаем аудио отдельно
    ...
    merge_inputs = ["-i", str(work_dir / "norm_vid.mp4")]
    ...
    (work_dir / "norm_vid.mp4").unlink(missing_ok=True)
    audio_path.unlink(missing_ok=True)

    if not norm_path.exists() or norm_path.stat().st_size < 10000:
        raise RuntimeError("Не удалось нормализовать видео. Файл повреждён.")
