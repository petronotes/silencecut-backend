...
def normalize_to_mkv(input_path: Path, work_dir: Path) -> Path:
    ...
    # Samsung хранит 1080x1920 но с rotation=90 — берём как есть, не поворачиваем
    ...
    # Шаг A: декодируем видео в rawvideo файл
    cmd_vid = [
        "ffmpeg", "-y",
        ...
        "-f", "rawvideo",
        "-pix_fmt", "yuv420p",
        "-vf", f"scale={w}:{h}",       # точный размер без rotation
        "-r", "30",
        "-vsync", "cfr",
        str(raw_video),
    ]
